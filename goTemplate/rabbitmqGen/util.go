...
func genFile(c fileGenConfig) error {
	fp, created, err := utilX.MaybeCreateFile(c.dir, c.subdir, c.filename)
	if err != nil {
		return err
	}
	if !created {
...
func getListenerConfig(api *spec.ApiSpec) []string {
	...
	return listenerNames.KeysStr()
}

func getListenerAuths(api *spec.ApiSpec) []string {
	listenerAuthNames := collection.NewSet()
	for _, g := range api.Service.Groups {
		for _, h := range g.Routes {
			l := fmt.Sprintf("%sRabbitmqConf rabbitmq.RabbitListenerConf", h.Listener)
			listenerAuthNames.Add(l)
		}
	}
	return listenerAuthNames.KeysStr()
}
...
func getListenerAdditions(api *spec.ApiSpec) []string {
	listenerAdditionNames := collection.NewSet()
	for _, g := range api.Service.Groups {
		for _, h := range g.Routes {
			...
			l := fmt.Sprintf("server.Add(%s.%s(ctx, serverCtx.Config.%sRabbitmqConf, serverCtx))", pkgName, handler, h.Listener)
			listenerAdditionNames.Add(l)
		}
	}

	return listenerAdditionNames.KeysStr()
}
