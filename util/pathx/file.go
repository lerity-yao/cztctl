...
// RegisterCztctlHome register goctl home path.
func RegisterCztctlHome(home string) {
	goctlHome = home
}
...
// GetCztctlHome returns the path value of the goctl, the default path is ~/.goctl, if the path has
// been set by calling the RegisterGoctlHome method, the user-defined path refers to.
func GetCztctlHome() (home string, err error) {
	defer func() {
		if err != nil {
			return
		}
