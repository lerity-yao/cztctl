...
// checkServiceStmt
// checkServiceStmt todo something yaox
func (api *API) checkServiceStmt() error {
	...
	listenerChecher := f.addCheckItem(api.Filename, "listener expression")
	...
	for _, v := range api.ServiceStmts {
		...
		for _, item := range v.Routes {
			listenerChecher.checkNodeWithPrefix(group, item.AtListener.Name)
