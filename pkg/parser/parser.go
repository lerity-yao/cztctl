...
// parseService
// parseService todo something yaox
...
// parseServiceItemStmt
// parseServiceItemStmt todo something yaox
func (p *Parser) parseServiceItemStmt() *ast.ServiceItemStmt {
	...
	// statement @handler
	// todo something p.advanceIfPeekTokenIs(token.AT_HANDLER, token.RBRACE) -> p.advanceIfPeekTokenIs(token.AT_LISTENER, token.RBRACE)
	if !p.advanceIfPeekTokenIs(token.AT_LISTENER, token.RBRACE) {
		return nil
	}
	...
	atListenerStmt := p.parseAtListenerStmt()
	if atListenerStmt == nil {
		return nil
	}
	stmt.AtListener = atListenerStmt
...
func (p *Parser) parserQueueStmt() *ast.QueueStmt {
	stmt := ast.QueueStmt{
	...
	for p.curTokenIsNotEof() &&
		p.peekTokenIsNot(token.RBRACE, token.LPAREN, token.Returns, token.AT_DOC, token.AT_HANDLER, token.SEMICOLON, token.RBRACE, token.AT_LISTENER) {
		// token '/'
		if !p.advanceIfPeekTokenIs(token.QUO) {
			return nil
		}
		position = p.curTok.Position
		if p.peekTokenIs(token.LPAREN, token.Returns, token.AT_DOC, token.AT_HANDLER, token.SEMICOLON, token.RBRACE, token.AT_LISTENER) {
			break
		}
		...
		// token ':' or IDENT
		if p.notExpectPeekToken(token.COLON, token.IDENT, token.INT, token.DOT) {
			return nil
		}
		...
		for p.curTokenIsNotEof() {
			...
			if p.expectPeekToken(token.COLON, token.IDENT, token.INT, token.DOT, token.QUO, token.RBRACE) {
				if p.curTokenIsNot(token.QUO) {
					queue = queue + p.curTok.Text
				}
			}
...
// parseAtListenerStmt
// parseAtListenerStmt todo something yaox
func (p *Parser) parseAtListenerStmt() *ast.AtListenerStmt {
	var stmt = &ast.AtListenerStmt{}
	stmt.AtListener = p.curTokenNode()
