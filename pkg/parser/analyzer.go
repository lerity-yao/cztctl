...
func (a *Analyzer) fillService() error {
	...
	for _, item := range a.api.ServiceStmts {
		...
		for _, astRoute := range item.Routes {
			head, _ := astRoute.CommentGroup()
			route := spec.Route{
				Queues:  astRoute.Queues.Queues,
				Doc:     head.List(),
				Comment: nil,
			}
			...
			if astRoute.AtListener != nil {
				route.AtDoc = a.convertAtDoc(astRoute.AtDoc)
				head, leading := astRoute.AtListener.CommentGroup()
				route.ListenerDoc = head.List()
				route.ListenerComment = leading.List()
				route.Listener = astRoute.AtListener.Name.Token.Text
			}
